...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    def save_message(self, message, sender, receiver,date,time,ai):
        return ChatMsg.objects.create(message=message, sender=sender, receiver=receiver,created_at_date=date,created_at_time=time,ai=ai)
    ...
    def save_group_message(self,message, sender, receiver, date, time,ai):
        return GroupMessage.objects.create(message=message, sender=sender, group=receiver, created_at_date=date, created_at_time=time,ai=ai)
    ...
    async def send_group_message(self, message, sender, receiver,ai):
        ...
        message = await self.save_group_message(message, sender, receiver, date, time,ai)
        ...
        if ai:
            ...
            date = getdate()
            time = gettime()
            answer = handler.handle_context_required_message(message.message)
    ...
    async def send_individual_message(self, message, sender, receiver,ai):
        ...
        if ai:
                ...
                date = getdate()
                time = gettime()
                answer = handler.handle_context_required_message(message)
