...
class MessageHandler:
    ...
    def __is__question__answerable__(self, question):
        response = openai.Completion.create(
            ...
            prompt=f"Is this question answerable by any of the above roles {self.roles}? The question is as follows: {question} . Just Return Yes Or No Without Full Stop",
            max_tokens=50,
        )
    ...
    def is_question_project_related(self,question):
        response = openai.Completion.create(
            engine="gpt-3.5-turbo-instruct",
            prompt=f"Will this question require some context before you can answer it? The question is as follows: {question}. Just Return Yes Or No Without Full Stop",
            max_tokens=50,
        )
    ...
    def handle_message(self, user_question):
        ...
        if role == None:
            ...
            if self.is_question_project_related(user_question):
                print('question is project related')
                response = self.handle_context_required_message(user_question)
            else:
                print('question is not project related')
                response = openai.Completion.create(
                    engine="gpt-3.5-turbo-instruct",
