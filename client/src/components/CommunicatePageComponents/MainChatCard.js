...
export default function MainChatCard({ id }) {
  ...
  useEffect(() => {
    ...
    // Cleaning up the WebSocket connection when the component unmounts
    return () => {
    ...
    <div className="border border-gray-200 bg-white w-3/4 h-screen relative">
      <div className="w-full flex items-start justify-start p-2 border border-gray-200 absolute top-0 bg-white z-1">
        ...
        <div className="flex flex-col px-4 justify-center items-start text-gray-600">
          ...
          <h3 className="text-base font-light">
            Last active at: 3rd Nov 2023, 7:08PM
          </h3>
        </div>
      </div>
      ...
      <div className="flex justify-center items-center border border-gray-200 p-3 absolute bottom-0 bg-white w-full">
        <input
          ...
          className="mx-4 border border-gray-200 h-10 w-full rounded-lg px-2 text-black"
        />
        <button
        ...
        >
          <IoSend className="text-black mx-4 w-8 h-8" />
        </button>
        {Group && (
          <button
            id="ai-grp-btn"
            ...
            onClick={(e) => {
              ...
              if (
                ...
                setAI(true);
                document.getElementById("ai-grp-btn").className =
                  "text-blue-600";
              }
            }}
          >
            <GoCopilot className=" transition-all fade-in-out mx-4 w-8 h-8" />
          </button>
        )}
      </div>
    </div>
