      ...
      <section className="bg-[#F7F7F7] text-gray-600 body-font">
        <div className="container justify-center flex px-5 py-20 md:flex-row flex-col items-center">
          ...
          <div className="lg:flex-grow md:w-1/2 m-5 p-5 flex flex-col md:items-start md:text-left items-center text-center bg-white shadow-lg rounded-md">
            <h1 className="title-font sm:text-3xl text-xl mb-4 font-medium text-gray-500">
              {EachProject?.data?.title}
            </h1>
            <p className="mb-8 leading-relaxed">
              {EachProject?.data?.description}
            </p>
            {/* Conditional rendering of buttons based on user and project status */}
            {EachProject?.data?.created_by?.user?.id != auth.user.id ? (
              <>
                {EachProject?.data?.status != "Open" &&
                  EachProject?.data?.team?.members?.some(
                    (x) => x.id === auth.user.id
                  ) && (
                    <div className="grid grid-cols-2 justify-center text-center">
                      <>
                        {EachProject?.data?.workflow === null && (
                          <button
                            className=" text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg m-5 text-center flex justify-center items-center"
                            onClick={() => {
                              setIsOpen(true);
                              axios
                                .post(
                                  "http://103.159.214.229/api/v1/__send__generated__workflow__/",
                                  {
                                    id: auth.user.id,
                                    project_id: EachProject?.data?.id,
                                  }
                                )
                                .then((response) => {
                                  EachProject.refetch();
                        ...
                        {(EachProject?.data?.Learning_resources == null ||
                          EachProject?.data?.Learning_resources?.length ==
                            0) && (
                          <button
                            onClick={() => {
                              setIsOpen(true);
                              axios
                                .post(
                                  "http://103.159.214.229/api/v1/__learning__resource__/",
                                  {
                                    id: auth.user.id,
                                    project_id: EachProject?.data?.id,
                                  }
                        ...
                        {EachProject?.data?.prd != null &&
                          (EachProject?.data?.project_management == null ||
                            EachProject?.data?.project_management.length == 0 ||
                            ...
                            <button
                              onClick={() => {
                                ...
                                axios
                                  .post(
                                    "http://103.159.214.229/api/v1/__project__management__/",
                                    {
                                      id: auth.user.id,
                                      project_id: EachProject?.data?.id,
                                    }
                              ...
                              className=" text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg m-5 items-center"
                            >
                              Generate Project Mangement
                            </button>
                          )}
                      </>
                    </div>
                  ...
                  {EachProject?.data?.prd == null && (
                    <button
                      onClick={() => {
                        ...
                        axios
                          .post(
                            "http://103.159.214.229/api/v1/__send__generated__prd__/",
                            {
                              id: auth.user.id,
                              project_id: EachProject?.data?.id,
                            }
        ...
        {EachProject?.data?.workflow && (
          <div className="mx-10 my-10 text-black">
            <div
              ...
              className="flex my-5 items-center justify-between bg-white rounded-md shadow-lg p-2 px-5 hover:bg-[#E9F3FF] hover:border hover:border-blue-500 text-gray-500 hover:text-gray-800"
            >
              <h1 className="text-center font-semibold text-lg md:text-xl lg:text-3xl px-5">
                Workflow For Talents{" "}
              </h1>
              <div className="">
                {workflowOpen ? (
                  <svg
            ...
            {workflowOpen ? (
              <div className="bg-white p-10 shadow-lg rounded-md text-gray-500">
                {EachProject.isSuccess
                  ? parse(
                      EachProject.data.workflow
                        ? EachProject.data.workflow.description
                        : ""
                    )
                  : ""}
              </div>
        ...
        {EachProject?.data?.Learning_resources && (
          <div className="mx-10 my-10">
            <div
              onClick={learningResourcesClick}
              className="flex my-5 items-center justify-between bg-white rounded-md shadow-lg p-2 px-5 hover:bg-[#E9F3FF] hover:border hover:border-blue-500 text-gray-500 hover:text-gray-800"
            >
              <h1 className="text-center font-semibold px-5 text-lg md:text-xl lg:text-3xl">
                Learning Resources For Talents
              </h1>
              <div className="">
            ...
            {learningResourcesOpen ? (
              <div
                className="bg-white p-10 shadow-lg rounded-md text-gray-500"
        ...
        {EachProject?.data?.project_management && (
          <div className="mx-10 my-10">
            ...
            {projectManagementOpen ? (
              <div className="grid grid-cols-1 justify-center bg-white p-10 shadow-lg rounded-md text-gray-500">
                <WorkflowCard
                  workflowData={EachProject?.data?.project_management}
                />
                {}
              </div>
            ) : (
...
const fetchEachProject = async (id) => {
  return axios
    .get(`http://103.159.214.229/api/v1/__get__each__project__/${id}`)
    .then((response) => {
